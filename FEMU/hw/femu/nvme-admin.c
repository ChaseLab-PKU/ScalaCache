...
static uint16_t nvme_del_sq(FemuCtrl *n, NvmeCmd *cmd)
{
    ...
    if (!qid || nvme_check_sqid(n, qid)) {
        printf("nvme_del_sq: invalid qid\n");
        assert(0);
...
static uint16_t nvme_del_cq(FemuCtrl *n, NvmeCmd *cmd)
{
    ...
    if (!qid || nvme_check_cqid(n, qid)) {
        printf("nvme_del_cq: invalid cqid %u\n", qid);
        assert(0);
        return NVME_INVALID_CQID | NVME_DNR;
    }
    ...
    n->ext_ops.sum_stat(n, cq);
    n->active_cq--;
    if (n->active_cq == 1) {
        n->ext_ops.print_stat(n);
        n->ext_ops.reset_stat(n);
    }
...
static void clear_hmb(FemuCtrl *n) {
    // FIXME: ensure no pg_cc_rw_cmd is processed before clear
    // hack to ensure the correctness of ftl
    free(n->g_hmb_desptr);
    n->g_hmb_desptr = NULL;
    n->g_hmb_desptr_size = 0;
    n->ext_ops.pg_cc_clear(n);
}

static void hmb_initiate(FemuCtrl *n, bool mr, bool ehm, uint32_t hsize, uint64_t hmdla, uint64_t hmdlec) {

    uint32_t hmb_size = 0;

    assert(ehm == true);

    uint8_t *buffer = malloc(sizeof(uint8_t) * hsize * HMB_DESPTR_SIZE);

    femu_debug("hmb_initiate: hsize = %d, hmdla = %lx, hmdlec = %lx\n", hsize, hmdla, hmdlec);

    nvme_addr_read(n, hmdla, buffer, hmdlec * HMB_DESPTR_SIZE);

    if (n->g_hmb_desptr) {
        clear_hmb(n);
    }

    n->g_hmb_desptr = malloc(sizeof(HMBEntryDesptr) * hmdlec);
    n->g_hmb_desptr_size = 0;

    for (int i = 0; i < hmdlec; i++) {
        femu_debug("hmb_initiate: i = %d\n", i);
        uint64_t vir_addr = le64_to_cpu(*(uint64_t *)(buffer + i * HMB_DESPTR_SIZE));
        uint64_t phy_addr = le64_to_cpu(*(uint64_t *)(buffer + i * HMB_DESPTR_SIZE + 8));
        uint32_t bsize = le32_to_cpu(*(uint32_t *)(buffer + i * HMB_DESPTR_SIZE + 16));

        n->g_hmb_desptr[i] = (HMBEntryDesptr){.vir_addr = vir_addr, .phy_addr = phy_addr, .size = bsize, 
                                              .ID = (n->g_hmb_desptr_size++), .start_pg_id = 0};

        hmb_size += bsize;

        femu_debug("hmb_initiate: vir_addr = %lx, phy_addr = %lx, bsize = %d\n", vir_addr, phy_addr, bsize);
    }

    n->page_cache_size = hmb_size;

    femu_debug("hmb_intiate: pg_cc_init begin\n");

    if (n->ext_ops.pg_cc_init != NULL) {
        n->ext_ops.pg_cc_init(n);
    } else {
        femu_err("hmb_initiate: pg_cc_init is NULL\n");
        assert(0);
    }
    
    femu_debug("hmb_initiate finished\n");
}
...
static uint16_t nvme_set_feature(FemuCtrl *n, NvmeCmd *cmd, NvmeCqe *cqe)
{
    ...
    case NVME_HOST_MEMORY_BUFFER:
        n->features.host_mem_buf = 0x1;
        hmb_initiate(n, (bool)(dw11 & 0x2), (bool)(dw11 & 0x1), dw12, (uint64_t)dw13 + ((uint64_t)dw14 << 32), dw15);
        break;
    default:
        return NVME_INVALID_FIELD | NVME_DNR;
    }
