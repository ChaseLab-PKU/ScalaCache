...
enum NvmeIoCommands {
    ...
    NVME_CMD_WRITE              = 0x01,
    NVME_CMD_READ               = 0x02,
    NVME_CMD_UN_PIN_RW          = 0x03,
    NVME_CMD_WRITE_UNCOR        = 0x04,
    NVME_CMD_COMPARE            = 0x05,
    NVME_CMD_WRITE_ZEROES       = 0x08,
    NVME_CMD_DSM                = 0x09,
    NVME_CMD_PG_CC_WRITE        = 0x16,
    NVME_CMD_PG_CC_READ         = 0x17,
    NVME_CMD_FTL_SHUTDOWN       = 0x18,
...
enum PG_CC_CACHE_STAT {
    PG_CC_CACHE_READ_HIT             = 0x00,
    PG_CC_CACHE_READ_MISS            = 0x01,
    PG_CC_CACHE_WRITE_HIT            = 0x02,
    PG_CC_CACHE_WRITE_MISS           = 0x03,
    PG_CC_CACHE_READ                 = 0x04,
    PG_CC_CACHE_WRITE                = 0x05,
    PG_CC_CACHE_UNPIN                = 0x06,
    PG_CC_CACHE_RW_ALL                  = 0x07,
    PG_CC_CACHE_ENUM_COUNT,
};
...
#define PG_CC_INVALID_PAGE_ID (~(0ULL))
#define PG_CC_INVALID_VIR_ADDR (~(0ULL))
#define PG_CC_INVALID_PHY_ADDR (~(0ULL))
#define PG_CC_INVALID_LPA (~(0ULL))
...
#define NVME_MAX_REQUEST_SIZE (512)
...
typedef struct NvmeLBARangeEntry {
    uint64_t slba;
    uint64_t nlb;
    QTAILQ_ENTRY(NvmeLBARangeEntry) entry;
} NvmeLBARangeEntry;

typedef struct NvmeSubRequest {
    QTAILQ_HEAD(, NvmeLBARangeEntry) lba_list;
    uint64_t    lba_count;
    uint16_t    is_write;
    uint16_t    is_unpin;
    uint64_t    total_lat;
    uint64_t    nand_lat;
    uint64_t    send_ftl_time;
    uint64_t    rev_ftl_time;
    uint64_t    send_poller_time;
    uint64_t    rev_poller_time;
    uint64_t    expire_time;
    struct NvmeRequest *parent_req;
    QTAILQ_ENTRY(NvmeSubRequest) entry;
    size_t      pos;
    uint64_t    ftl_time;
    uint16_t    pg_cc_hit_num;
    uint16_t    pg_cc_miss_num;

    bool       *unpin_is_dirty;
    uint64_t   *unpin_pg_id;
    uint64_t    unpin_pg_num;
} NvmeSubRequest;
...
typedef struct NvmeRequest {
    ...
    int64_t                 pg_cc_time;
    int64_t                 nand_time;
    int64_t                 pg_cc_hit_num;
    int64_t                 pg_cc_miss_num;
    int64_t                 start_time;
    int64_t                 end_time;
    int64_t                 diff_time;
    int64_t                 to_ftl_time, to_poller_time;

    int64_t                 total_need_pin_num;
...
typedef struct NvmeSQueue {
    ...
    uint64_t    *prp_list;
    NvmeRequest *io_req;
    NvmeSubRequest *io_subReq;
    NvmeLBARangeEntry *io_lba_entry;
    QTAILQ_HEAD(sq_req_list, NvmeRequest) req_list;
    QTAILQ_HEAD(sq_subreq_list, NvmeSubRequest) subReq_list;
    QTAILQ_HEAD(out_req_list, NvmeRequest) out_req_list;
    QTAILQ_HEAD(sq_lba_list, NvmeLBARangeEntry) lba_list;
...
typedef struct NvmeCQueue {
...
#if 0
    QTAILQ_HEAD(cq_pg_cc_list, NvmeRequest) pg_cc_req_list;
#endif
    uint64_t    db_addr;
    uint64_t    db_addr_hva;
    uint64_t    eventidx_addr;
    ...
    double          sq_time[PG_CC_CACHE_ENUM_COUNT];
    double          cq_time[PG_CC_CACHE_ENUM_COUNT];
    double          ftl_time[PG_CC_CACHE_ENUM_COUNT];
    double          nand_time[PG_CC_CACHE_ENUM_COUNT];
    double          diff_time[PG_CC_CACHE_ENUM_COUNT];
    double          total_time[PG_CC_CACHE_ENUM_COUNT];
    double          to_ftl_time[PG_CC_CACHE_ENUM_COUNT];
    double          to_poller_time[PG_CC_CACHE_ENUM_COUNT];
    int64_t         req_num[PG_CC_CACHE_ENUM_COUNT];
    int64_t         hit_pg_num[PG_CC_CACHE_ENUM_COUNT];
    int64_t         miss_pg_num[PG_CC_CACHE_ENUM_COUNT];
    int64_t         pg_num[PG_CC_CACHE_ENUM_COUNT];
} NvmeCQueue;
...
typedef struct HMBEntryDesptr {
  uint64_t vir_addr;
  uint64_t phy_addr;
  uint32_t size;
  uint32_t ID;
  uint32_t start_pg_id;
} HMBEntryDesptr;

#define HMB_DESPTR_SIZE (24)
...
typedef struct FemuExtCtrlOps {
    ...
    void     (*stat)(NvmeCQueue *, NvmeRequest *);
    void     (*sum_stat)(FemuCtrl *, NvmeCQueue *);
    void     (*print_stat)(FemuCtrl *);
    void     (*reset_stat)(FemuCtrl *);
} FemuExtCtrlOps;
...
typedef struct FemuCtrl {
    ...
    uint32_t    hmpre;
    uint32_t    hmmin;
    HMBEntryDesptr *g_hmb_desptr;
    uint32_t g_hmb_desptr_size;
    bool       pg_cc_enabled;
    uint32_t    pg_cc_hmpre;
    ...
    struct rte_ring **admin2ftl;
    struct rte_ring **ftl2admin;
    pqueue_t        **pq;
    QTAILQ_HEAD(cq_cpl_pq_head, NvmeRequest) *cq_cpl_list;
    bool            *should_isr;
    bool            poller_on;
    ...
    uint32_t        warm_up_ratio;

    double          sq_time[PG_CC_CACHE_ENUM_COUNT];
    double          cq_time[PG_CC_CACHE_ENUM_COUNT];
    double          ftl_time[PG_CC_CACHE_ENUM_COUNT];
    double          nand_time[PG_CC_CACHE_ENUM_COUNT];
    double          pg_cc_time[PG_CC_CACHE_ENUM_COUNT];
    double          total_time[PG_CC_CACHE_ENUM_COUNT];
    double          diff_time[PG_CC_CACHE_ENUM_COUNT];
    double          to_ftl_time[PG_CC_CACHE_ENUM_COUNT];
    double          to_poller_time[PG_CC_CACHE_ENUM_COUNT];
...
static inline uint64_t fast_transLBA2LPN(uint64_t lba)
{
    return lba >> 3;
}
