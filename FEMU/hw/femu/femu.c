...
static void __attribute__((unused)) nvme_clear_host_mem_buf(FemuCtrl *n)
{
...
static void nvme_clear_ctrl(FemuCtrl *n, bool shutdown)
{
    ...
    if (shutdown) {
        femu_log("shutting down NVMe Controller ...\n");
    } else {
        femu_log("disabling NVMe Controller ...\n");
    }

    if (shutdown) {
        femu_debug("%s,clear_guest_notifier\n", __func__);
        nvme_clear_virq(n);
        // nvme_clear_host_mem_buf(n);

        for (int ftl_id = 1; ftl_id <= n->num_ftl; ftl_id++) {
            NvmeRequest *req = malloc(sizeof(NvmeRequest));
            req->cmd_opcode = NVME_CMD_FTL_SHUTDOWN;
            femu_ring_enqueue(n->admin2ftl[ftl_id], (void *)&req, 1);
            while (rte_ring_empty(n->ftl2admin[ftl_id])) {}
            femu_ring_dequeue(n->ftl2admin[ftl_id], (void *)&req, 1);
            // printf("clear ftl %d\n", ftl_id);
            free(req);
        }
    }
...
static void nvme_init_ctrl(FemuCtrl *n)
{
    ...
    id->hmpre        = cpu_to_le32(n->hmpre);
    id->hmmin        = cpu_to_le32(n->hmmin);

    femu_debug("hmpre: %d, hmmin: %d\n", n->hmpre, n->hmmin);

    ((uint32_t *)id->vs)[0] = (n->num_ftl);
    assert(n->shard_length % 4096 == 0);
...
static void nvme_destroy_poller(FemuCtrl *n)
{
    ...
    for (int i = 1; i <= n->num_poller; i++) {
        ...
        for (int j = 1; j <= n->num_ftl; j++) {
            femu_ring_free(n->to_poller[i][j]);
            femu_ring_free(n->to_ftl[i][j]);
        }
...
static Property femu_props[] = {
    ...
    DEFINE_PROP_UINT32("num_poller", FemuCtrl, num_poller, 1),
    DEFINE_PROP_UINT8("stride", FemuCtrl, db_stride, 0),
    DEFINE_PROP_UINT8("aerl", FemuCtrl, aerl, 3),
    DEFINE_PROP_UINT8("acl", FemuCtrl, acl, 3),
    DEFINE_PROP_UINT32("ftl_num", FemuCtrl, num_ftl, 1),
    DEFINE_PROP_UINT32("warm_up_ratio", FemuCtrl, warm_up_ratio, 0),
    DEFINE_PROP_INT32("read_lat", FemuCtrl, read_lat, 40000),
    DEFINE_PROP_INT32("write_lat", FemuCtrl, write_lat, 200000),
    DEFINE_PROP_INT32("erase_lat", FemuCtrl, erase_lat, 2000000),
    ...
    DEFINE_PROP_UINT32("hmpre", FemuCtrl, hmpre, 0),
    DEFINE_PROP_UINT32("hmmin", FemuCtrl, hmmin, 0),
    DEFINE_PROP_UINT32("pg_cc_hmpre", FemuCtrl, pg_cc_hmpre, 1048576),
