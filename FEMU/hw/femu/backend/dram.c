...
void backend_pg_cc_rw(SsdDramBackend *b, pg_cc_dram_rw_context *ctx) {
    ...
    uint64_t *pg_addr = ctx->page_addr;
    uint64_t start_addr = ctx->start_addr;
    bool *need_cp = ctx->need_cp;

    if (ctx->is_write) {
        dir = DMA_DIRECTION_TO_DEVICE;
    }

    for (int i = 0; i < ctx->page_num; i++) {
        if (need_cp[i]) {
            assert(pg_addr[i] != PG_CC_INVALID_PHY_ADDR);
            if (dma_memory_rw(ctx->as, pg_addr[i], mb + start_addr, 4096, dir, MEMTXATTRS_UNSPECIFIED)) {
