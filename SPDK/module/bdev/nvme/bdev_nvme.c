...
static int bdev_nvme_no_pi_readv(struct nvme_bdev_io *bio, struct iovec *iov, int iovcnt,
				 void *md, uint64_t lba_count, uint64_t lba);
static int bdev_nvme_unpin(struct nvme_bdev_io *bio, uint64_t lba_count, uint64_t lba, uint16_t origin_opc);
...
bdev_nvme_submit_request(struct spdk_io_channel *ch, struct spdk_bdev_io *bdev_io)
{
	...
	case SPDK_BDEV_IO_TYPE_WRITE:
		rc = bdev_nvme_writev(nbdev_io,
				      ...
				      bdev_io->u.bdev.ext_opts);
		break;
	case SPDK_BDEV_IO_TYPE_UNPIN:
		rc = bdev_nvme_unpin(nbdev_io,
				     bdev_io->u.bdev.num_blocks,
				     bdev_io->u.bdev.offset_blocks,
						 bdev_io->u.bdev.unpin.origin_opc);
		break;
...
static void
nvme_ctrlr_set_host_memory_buffer_done(void *_ctx, const struct spdk_nvme_cpl *cpl)
{
	struct nvme_ctrlr *ctrlr = _ctx;
	struct nvme_async_probe_ctx *ctx = ctrlr->probe_ctx;

	ctrlr->probe_ctx = NULL;

	if (spdk_nvme_cpl_is_error(cpl)) {
		SPDK_ERRLOG("Set Host Memory Buffer failed\n");
		assert(0);
	}

	nvme_ctrlr_create_done(ctrlr, ctx);
}

static void
nvme_ctrlr_use_existed_host_memory_buffer_done(void *_ctx) {
	struct nvme_ctrlr *ctrlr = _ctx;
	struct nvme_async_probe_ctx *ctx = ctrlr->probe_ctx;
...
static int
nvme_ctrlr_cmd_set_host_memory_buffer(struct nvme_ctrlr *nvme_ctrlr, struct nvme_async_probe_ctx *ctx)
{
	struct spdk_nvme_ctrlr *ctrlr = nvme_ctrlr->ctrlr;
	nvme_ctrlr->probe_ctx = ctx;

	int rc = spdk_nvme_ctrlr_cmd_set_host_memory_buffer(ctrlr,
						nvme_ctrlr_set_host_memory_buffer_done,
						nvme_ctrlr);

	if (rc == 1) {
		// TODO: HACK TO CHECK IF THE BUFFER IS ALREADY SET
		nvme_ctrlr_use_existed_host_memory_buffer_done(nvme_ctrlr);
		return 0;
	}
		  ...
		  struct nvme_async_probe_ctx *ctx)
{
	...
	if (cdata->cmic.ana_reporting) {
		...
		rc = nvme_ctrlr_cmd_set_host_memory_buffer(nvme_ctrlr, ctx);
		if (rc == 0) {
			return 0;
		}
	} else {
...
static void
bdev_nvme_unpin_done(void *ref, const struct spdk_nvme_cpl *cpl)
{
	struct nvme_bdev_io *bio = ref;

	if (spdk_nvme_cpl_is_pi_error(cpl)) {
		SPDK_ERRLOG("unpin completed with PI error (sct=%d, sc=%d)\n",
			    cpl->status.sct, cpl->status.sc);
...
static int
bdev_nvme_unpin(struct nvme_bdev_io *bio, uint64_t lba_count, uint64_t lba, uint16_t origin_opc)
{
	struct spdk_nvme_ns *ns = bio->io_path->nvme_ns->ns;
	struct spdk_nvme_qpair *qpair = bio->io_path->qpair->qpair;
	int rc;

	SPDK_DEBUGLOG(bdev_nvme, "unpin %" PRIu64 " blocks with offset %#" PRIx64 "\n",
		      lba_count, lba);

	rc = spdk_nvme_ns_cmd_pg_cc_unpin(ns, qpair, lba, lba_count, origin_opc,
				    bdev_nvme_unpin_done, bio);

	if (rc != 0 && rc != -ENOMEM) {
		SPDK_ERRLOG("unpin failed: rc = %d\n", rc);
		assert(0);
	}
	return rc;
}
