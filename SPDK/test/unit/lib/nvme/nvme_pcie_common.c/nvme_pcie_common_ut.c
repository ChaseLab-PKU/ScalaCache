...
{
	return (struct unpacked_pte_header){false, false, 0, 0};
}
...
uint64_t spdk_nvme_ctrlr_trans_pgid2ftlid(const struct spdk_nvme_ctrlr *ctrlr, uint32_t pgid)
{
	return 0;
}
...
/*
static bool nvme_pcie_qpair_look_up_host_mem(struct spdk_nvme_ctrlr *ctrlr, struct spdk_nvme_qpair *qpair,
