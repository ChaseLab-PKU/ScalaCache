...
bool
spdk_bdev_has_page_cache(const struct spdk_bdev *bdev)
{
	return bdev->page_cache;
}
...
static int
bdev_unpin_blocks(struct spdk_bdev_desc *desc, struct spdk_io_channel *ch,
				uint64_t offset_blocks, uint64_t num_blocks, uint16_t origin_opc,
				spdk_bdev_io_completion_cb cb, void *cb_arg)
{
	struct spdk_bdev *bdev = spdk_bdev_desc_get_bdev(desc);
	struct spdk_bdev_io *bdev_io;
	...
	bdev_io->internal.ch = channel;
	bdev_io->internal.desc = desc;
	bdev_io->type = SPDK_BDEV_IO_TYPE_UNPIN;
	bdev_io->u.bdev.iovs = &bdev_io->iov;
	...
	bdev_io->u.bdev.unpin.origin_opc = origin_opc;
	bdev_io_init(bdev_io, bdev, cb_arg, cb);
...
int
spdk_bdev_unpin(struct spdk_bdev_desc *desc, struct spdk_io_channel *ch,
					uint64_t offset, uint64_t nbytes, uint16_t origin_opc,
					spdk_bdev_io_completion_cb cb, void *cb_arg)
{
	...
	return spdk_bdev_unpin_blocks(desc, ch, offset_blocks, num_blocks, origin_opc, cb, cb_arg);
}
...
int 
spdk_bdev_unpin_blocks(struct spdk_bdev_desc *desc, struct spdk_io_channel *ch,
								uint64_t offset_blocks, uint64_t num_blocks, uint16_t origin_opc,
								spdk_bdev_io_completion_cb cb, void *cb_arg)
{
	return bdev_unpin_blocks(desc, ch, offset_blocks, num_blocks, origin_opc, cb, cb_arg);
}
