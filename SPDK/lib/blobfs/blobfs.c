...
__start_cache_pool_mgmt(void *ctx)
{
	...
	SPDK_ERRLOG("before g_fs_cache_size = %lu\n", g_fs_cache_size);
	g_fs_cache_size = 4294967296 / 2;
	SPDK_ERRLOG("after g_fs_cache_size = %lu\n", g_fs_cache_size);
	g_cache_pool = spdk_mempool_create("spdk_fs_cache",
					   g_fs_cache_size / CACHE_BUFFER_SIZE,
					   CACHE_BUFFER_SIZE,
					   SPDK_MEMPOOL_DEFAULT_CACHE_SIZE,
					   SPDK_ENV_SOCKET_ID_ANY);
	SPDK_ERRLOG("finnal g_fs_cache_size = %lu\n", g_fs_cache_size);
	if (!g_cache_pool) {
		  ...
		  const char *name, uint32_t flags, struct spdk_file **file)
{
	...
	if (rc != 0) {
		SPDK_ERRLOG("Failed to open file=%s, rc=%d\n", name, rc);
	}
	free_fs_request(req);
