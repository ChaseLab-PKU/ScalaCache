				 ...
				 spdk_nvme_cmd_cb cb_fn, void *cb_arg)
{
	...
	/* hack to print stat when deleting io CQ*/

	spdk_print_statis(qpair);
				 ...
				 uint16_t qid)
{
	...
	if (!qpair->stat) {
		qpair->stat = calloc(1, sizeof(*qpair->stat));
		if (!qpair->stat) {
			SPDK_ERRLOG("Failed to allocate qpair->stat\n");
			assert(0);
		}
	}
...
static void
__attribute__((unused)) print_cmd(struct spdk_nvme_qpair *qpair, struct spdk_nvme_cmd *cmd)
{
	struct nvme_pcie_qpair	*pqpair = nvme_pcie_qpair(qpair);
	printf("sq id: %hu, sq head: %hu, sq tail: %hu, last_sq_tail: %hu, opc: %hu, cid: %hu, nsid: %u, qsize: %hu\n",
										qpair->id, pqpair->sq_head, pqpair->sq_tail, pqpair->last_sq_tail, cmd->opc, cmd->cid, cmd->nsid, pqpair->num_entries);
}

static void
nvme_pcie_qpair_construst_unpin_cmd(uint32_t *pg_id, uint32_t *is_dirty, uint32_t count, struct nvme_tracker *tr)
{
	struct nvme_request *req = tr->req;
	struct spdk_nvme_cmd *cmd = &req->cmd;

	cmd->rsvd2 = count;
	cmd->rsvd3 = 0;
	for (uint64_t i = 0; i < spdk_min(count, 6); i++) {
		assert(is_dirty[i] == 0 || is_dirty[i] == 1);
		cmd->rsvd3 |= (is_dirty[i] << i);
	}
	cmd->cdw10 = pg_id[0];
	if (count >= 2) {
		cmd->cdw11 = pg_id[1];
	...
	if (count > 6) {
		assert(req->cmd.dptr.prp.prp1 == tr->prp_sgl_bus_addr);
		for (uint32_t i = 6; i < count; i++) {
			tr->u.prp[i - 6] = ((uint64_t)pg_id[i]) | (((uint64_t)is_dirty[i]) << 32);
		}
	}
}

static void
nvme_pcie_set_time(uint64_t *time) {
	*time = spdk_get_ticks();
}

static void
nvme_pcie_init_stat(struct nvme_request *req, uint64_t pg_num) {
	nvme_pcie_set_time(&(req->stat.precache_start_time));
	req->stat.pg_num = pg_num;
}

static uint64_t
nvme_pcie_get_free_pg_num(struct spdk_nvme_qpair *qpair) {
	struct spdk_nvme_ctrlr *ctrlr = qpair->ctrlr;
	int ftl_num = spdk_nvme_ctrlr_get_ftl_num(ctrlr);
	uint64_t free_pg_num = 0;
	for (int i = 0; i < ftl_num; i++) {
		assert(qpair->mapping_size[i] >= qpair->pin_size[i]);
		free_pg_num += qpair->mapping_size[i] - qpair->pin_size[i];
	}
	return free_pg_num;
}

static void
nvme_pcie_qpair_peek_victim_for_pre_unpin_cmd(struct spdk_nvme_qpair *qpair, struct nvme_request *origin_req, struct nvme_tracker *new_tr)
{
	uint64_t lba = origin_req->cmd.cdw10;
	uint64_t lba_count = origin_req->cmd.cdw12 & 0xFFFF;
	...
	uint64_t nlpn = end_lpn - start_lpn + 1;

	/* TODO: fix the hack method of peeking page */
	/*
	while (count < batch_size) {
		pg_id[count] = peek_victim(qpair, &is_dirty[count]);
		count++;
	}
	*/
  ...
  for (uint64_t i = 0; i < ftl_num; i++) {
    if (need_pre_unpin[i]) {
			if (qpair->mapping_size[i] > cachesize_limit) {
				origin_req->need_pg_num[i] = spdk_min(origin_req->need_pg_num[i], qpair->mapping_size[i] - cachesize_limit);
				batch_size += origin_req->need_pg_num[i];
			} else {
				origin_req->need_pg_num[i] = 0;
			}
    } else {
			assert(origin_req->need_pg_num[i] == 0);
		}
  }
	batch_size = spdk_min(batch_size, NVME_MAX_PRP_LIST_ENTRIES + 6);
	assert(batch_size > 0);

	uint32_t pg_id[batch_size];
	uint32_t is_dirty[batch_size];

	nvme_pcie_init_stat(new_tr->req, batch_size);

	peek_batch_victim(qpair, pg_id, is_dirty, origin_req->need_pg_num, batch_size, start_lpn, nlpn);
	nvme_pcie_qpair_construst_unpin_cmd(pg_id, is_dirty, batch_size, new_tr);

}

static void
nvme_pcie_batch_unpin_cmd(struct spdk_nvme_qpair *qpair, struct nvme_tracker *tr)
{
	uint64_t batch_size = spdk_min(503 + 6, nvme_pcie_get_free_pg_num(qpair));
	uint32_t pg_id[batch_size];
	uint32_t is_dirty[batch_size];
	uint64_t count = 0;
	...
	nvme_pcie_qpair_construst_unpin_cmd(pg_id, is_dirty, batch_size, tr);
}

static bool
nvme_pcie_qpair_unpin_host_cmd(struct spdk_nvme_ctrlr *ctrlr, struct spdk_nvme_qpair *qpair,
					struct nvme_tracker *tr)
{
	struct nvme_request *req = tr->req;
	...
	uint64_t start_lpn = req_offset / 4096;
	uint64_t end_lpn = (req_offset + req_length - 1) / 4096;
	bool is_write = req->origin_opc == SPDK_NVME_OPC_PG_CC_WRITE;
	assert(req->origin_opc == SPDK_NVME_OPC_PG_CC_WRITE || req->origin_opc == SPDK_NVME_OPC_PG_CC_READ);
	struct page_table_cacheline *cacheline;
	struct unpacked_pte_header pte;

	nvme_pcie_init_stat(req, 0);

	for (uint64_t i = start_lpn; i <= end_lpn; i++) {
		cacheline = get_cacheline(qpair, i);
		pte = get_pte(qpair, i, cacheline);
		if (!pte.exist) {
			assert(0);
		} else {
			assert(!pte.busy);
			unpin_page(qpair, i, is_write);
		}
	}

	if (get_total_mapping_size(qpair) == spdk_nvme_ctrlr_get_cachesize_per_qpair(ctrlr)) {
		nvme_pcie_batch_unpin_cmd(qpair, tr);
		return false;
	} else {
		return true;
	}
...
static void
nvme_pcie_qpair_unpin_rw_callback(struct spdk_nvme_ctrlr *ctrlr, struct spdk_nvme_qpair *qpair, struct nvme_tracker *tr) {
	struct nvme_request *req = tr->req;
	uint64_t lba = req->cmd.cdw10;
	uint64_t lba_count = req->cmd.cdw12 & 0xFFFF;
	uint64_t req_length = (lba_count + 1) * 512;
	...
	uint64_t start_lpn = req_offset / 4096;
	uint64_t end_lpn = (req_offset + req_length - 1) / 4096;
	bool is_write = req->cmd.opc == SPDK_NVME_OPC_PG_CC_WRITE;
	assert(req->cmd.opc == SPDK_NVME_OPC_PG_CC_WRITE || req->cmd.opc == SPDK_NVME_OPC_PG_CC_READ);
	struct page_table_cacheline *cacheline;
	struct unpacked_pte_header pte;
...
static void
nvme_pcie_update_cache_hit_pg_num(struct nvme_request *req, uint64_t pg_num) {
	req->stat.pg_hit_num += pg_num;
}

static void
nvme_pcie_update_cache_miss_pg_num(struct nvme_request *req, uint64_t pg_num) {
	req->stat.pg_miss_num += pg_num;
}

static bool
nvme_pcie_qpair_look_up_host_mem(struct spdk_nvme_ctrlr *ctrlr, struct spdk_nvme_qpair *qpair,
					struct nvme_tracker *tr)
{
	static uint64_t req_count = 0;
	static uint64_t send_req_count = 0;
	struct nvme_request *req = tr->req;
	uint64_t lba = req->cmd.cdw10;
	uint64_t lba_count = req->cmd.cdw12 & 0xFFFF;
	uint64_t req_length = (lba_count + 1) * 512;
	...
	assert(shard_pg_num > 0);
	req->total_need_pg_num = 0;
	req->pending_pg_num = 0;
	nvme_pcie_init_stat(req, nlpn);
	memset(req->need_pg_num, 0, sizeof(uint16_t) * ftl_num);

	struct page_table_cacheline *cacheline;
	struct unpacked_pte_header pte;
	bool all_hit_in_driver = true;

	for (uint64_t i = start_lpn; i <= end_lpn; i++) {
		req->phy_pg_id[i - start_lpn] = INVALID_PG_ID;
	}
	assert(nlpn <= 504);
	...
	for (uint64_t i = start_lpn; i <= end_lpn; i++) {
		...
		if (!pte.exist) {
			...
			req->hit_in_driver[i - start_lpn] = false;
			all_hit_in_driver = false;
			req->need_pg_num[spdk_nvme_ctrlr_trans_pgid2ftlid(ctrlr, i)]++;
			req->total_need_pg_num++;
			cacheline = get_or_create_cacheline(qpair, i);
			assert(cacheline);
			create_mapping(qpair, i, INVALID_PG_ID, is_write, cacheline); // set busy bit

			nvme_pcie_update_cache_miss_pg_num(req, 1);
			// printf("busy req_count: %lu, s_lpn %lu, i: %lu, cacheline: id %lu, tag %lu, offset %lu\n",
			//					req_count, start_lpn, i, get_cacheline_pos(cacheline), (uint64_t)(cacheline_tag), cacheline_offset);
		} else {
			req->hit_in_driver[i - start_lpn] = true;
			// pin_page_with_hint(qpair, i, &pg_id, cacheline);
			pg_id = cacheline->ppage_ids[i % CACHELINE_PACK_PAGES];
			if (is_write)
				cacheline->header[i % CACHELINE_PACK_PAGES].dirty = is_write;

			if (pte.busy) {
				uint64_t cacheline_offset = i % CACHELINE_PACK_PAGES;
				uint64_t cacheline_tag = i / CACHELINE_PACK_PAGES;
				assert(cacheline && cacheline->tag == cacheline_tag);
				pending_req_push_back(qpair, get_cacheline_pos(cacheline), cacheline_offset, req);
				req->pending_pg_num++;
				nvme_pcie_update_cache_hit_pg_num(req, 1);
				// printf("pending req_count: %lu, s_lpn %lu, i: %lu, cacheline: id %lu, tag %lu, offset %lu\n",
				// 				req_count, start_lpn, i, get_cacheline_pos(cacheline), (uint64_t)(cacheline_tag), cacheline_offset);
			} else {
				req->phy_pg_id[i - start_lpn] = pg_id;
				nvme_pcie_pg_cc_copy_single_page(qpair, req, i - start_lpn);
				assert(i - start_lpn <= 504);
				nvme_pcie_update_cache_hit_pg_num(req, 1);
			}
		}
	}

	if (!all_hit_in_driver) {
		/* TODO: hack to set pending_pg_num for the requests sent to SSD */
		req->pending_pg_num++;

		if (nlpn <= 31) {
			req->cmd.rsvd2 = 0;
			for (uint64_t i = 0; i < nlpn; i++) {
				req->cmd.rsvd2 |= (req->hit_in_driver[i] << i);
			}
		} else {
			uint64_t need_pin_pg_num = 0;
			uint64_t prp_index = 0, prp_offset = 0;
			uint64_t biset_size = 64;
			for (uint64_t i = 0; i < ((nlpn - 1) / biset_size) + 1; i++) {
				tr->u.prp[i] = 0;
			}
			for (uint64_t i = 0; i < nlpn; i++) {
				prp_index = i / biset_size;
				prp_offset = i % biset_size;
				if (req->hit_in_driver[i]) {
					tr->u.prp[prp_index] |= (1ULL << prp_offset);
				} else {
					need_pin_pg_num++;
				}
			}
			req->cmd.rsvd2 = need_pin_pg_num;
		}
	}
	...
	if (!all_hit_in_driver) {
		...
		// req_count++;
		if (req_count % 1000000 == 0) {
			// printf("rq: %lu\n", req_count);
		}
#if 0
		if (req_count > 490000) {
			printf("rq: %lu %lu %lu %lu\n", req_count, start_lpn, nlpn, spdk_nvme_ctrlr_get_cachesize_per_qpair(ctrlr) - qpair->mapping_size);
		}

#endif
		if (send_req_count >= 52000) {
			// printf("rq: %lu %lu %lu %lu\n", req_count, start_lpn, nlpn, spdk_nvme_ctrlr_get_cachesize_per_qpair(ctrlr) - qpair->mapping_size);
		}
	...
	return all_hit_in_driver;
}

static bool
is_pg_cc_io_request(uint16_t opc) {
	return (opc == SPDK_NVME_OPC_PG_CC_READ || opc == SPDK_NVME_OPC_PG_CC_WRITE);
}

#if 0
static uint16_t
tran_io_request(uint16_t opc) {
	if (opc == SPDK_NVME_OPC_PG_CC_READ)
		return SPDK_NVME_OPC_READ;
	else if (opc == SPDK_NVME_OPC_PG_CC_WRITE)
		return SPDK_NVME_OPC_WRITE;
	else
		return opc;
}
#endif
...
static void
nvme_pcie_qpair_submit_pre_unpin_cmd(struct spdk_nvme_qpair *qpair, struct nvme_request *origin_req)
{
	struct nvme_request *req;
	struct nvme_tracker *tr;
	struct nvme_pcie_qpair	*pqpair = nvme_pcie_qpair(qpair);
	...
	req = nvme_allocate_request(qpair, &payload, 0, 0, NULL, NULL);
	assert(req);
	req->is_pre_unpin = true;

	req->payload_offset = 0;
	req->md_offset = 0;
	
	cmd = &req->cmd;
	cmd->opc = SPDK_NVME_OPC_UN_PIN;
	cmd->nsid = origin_req->cmd.nsid;
	...
	req->cmd.cid = tr->cid;
	cmd->dptr.prp.prp1 = tr->prp_sgl_bus_addr;

	nvme_pcie_qpair_peek_victim_for_pre_unpin_cmd(qpair, origin_req, tr);
	nvme_pcie_qpair_submit_tracker(qpair, tr);
}

static void
nvme_pcie_qpair_pre_pin_pg(struct spdk_nvme_qpair *qpair, struct nvme_request *req)
{
	uint64_t ftl_num = spdk_nvme_ctrlr_get_ftl_num(qpair->ctrlr);
	for (uint64_t i = 0; i < ftl_num; i++) {
		qpair->mapping_size[i] += req->need_pg_num[i];
	}
}
...
nvme_pcie_qpair_submit_tracker(struct spdk_nvme_qpair *qpair, struct nvme_tracker *tr)
{
	...
	if (ctrlr->host_mem_size && !nvme_qpair_is_admin_queue(qpair)) {
		
		nvme_pcie_set_time(&(req->stat.req_sending_end_time));

		if (is_pg_cc_io_request(req->cmd.opc)) {
			if (nvme_pcie_qpair_look_up_host_mem(ctrlr, qpair, tr)) {
				if (req->pending_pg_num == 0) {
					STAILQ_INSERT_HEAD(&qpair->hit_req, req, hit_stailq);
				}
				nvme_pcie_set_time(&(req->stat.precache_end_time));
				nvme_pcie_set_time(&(req->stat.req_pending_start_time));
				return ;
			} else {
				nvme_pcie_qpair_pre_pin_pg(qpair, req);
				if (check_need_pre_unpin(qpair, req)) {
					nvme_pcie_qpair_submit_pre_unpin_cmd(qpair, req);
				}
			}
		} else if (req->cmd.opc == SPDK_NVME_OPC_UN_PIN) {
			if (!req->is_pre_unpin && nvme_pcie_qpair_unpin_host_cmd(ctrlr, qpair, tr)) {
				STAILQ_INSERT_HEAD(&qpair->hit_req, req, hit_stailq);
				nvme_pcie_set_time(&(req->stat.precache_end_time));
				nvme_pcie_set_time(&(req->stat.req_pending_start_time));
				return ;
...
static void nvme_pcie_copy_pg_cc(pthread_rwlock_t *rwlock, void *virt_addr, void *pg_cc_addr, uint32_t len, bool is_write) {
	/*
	if (!is_write) {
	...
	*/
	...
	if (is_write) {
		spdk_dma_memcpy(pg_cc_addr, virt_addr, len);
		// memcpy(pg_cc_addr, virt_addr, len);
		SPDK_DEBUGLOG(nvme, "nvme_pcie_copy_pg_cc: from virt_addr:%p to pg_cc_addr=%p, len=%d\n",
						virt_addr, pg_cc_addr, len);
	} else {
		spdk_dma_memcpy(virt_addr, pg_cc_addr, len);
		// memcpy(virt_addr, pg_cc_addr, len);
		SPDK_DEBUGLOG(nvme, "nvme_pcie_copy_pg_cc: from pg_cc_addr:%p to virt_addr=%p, len=%d\n",
						pg_cc_addr, virt_addr, len);
	}
	/*
...
static void nvme_pcie_pg_cc_copy_prp_single_page(struct spdk_nvme_qpair *qpair, struct nvme_request *req, uint64_t nlpn, uint64_t req_length, uint64_t req_offset, uint64_t lpn_index) {
	uint64_t pg_cc_addr;
	uint64_t len = req->payload_size;
	uint32_t page_len, page_offset;
	uint32_t page_size = 4096; 
	uintptr_t page_mask = page_size - 1;
	bool is_write = req->cmd.opc == SPDK_NVME_OPC_PG_CC_WRITE;

	uintptr_t virt_addr = (uintptr_t)req->payload.contig_or_cb_arg + req->payload_offset;
	uint64_t req_total_size = req->parent ? req->parent->payload_size : req->payload_size;

	if (lpn_index != 0) {
		uint64_t forward_length = (page_size - (req_offset & page_mask)) + (lpn_index - 1) * page_size;
		virt_addr += forward_length;
		req_offset += forward_length;
		len -= forward_length;
		assert((req_offset & page_mask) == 0);
	}

	page_len = spdk_min(page_size - ((uintptr_t)req_offset & page_mask), len);
	page_offset = (uintptr_t)req_offset & page_mask;

	pg_cc_addr = trans_pgid2addr(qpair->ctrlr, req->phy_pg_id[lpn_index]);

	assert(virt_addr >= (uint64_t)req->payload.contig_or_cb_arg + req->payload_offset);
	assert(virt_addr + page_len <= ((uint64_t)req->payload.contig_or_cb_arg) + req_total_size);

	nvme_pcie_copy_pg_cc(&qpair->ctrlr->host_mem_pg_lock[req->phy_pg_id[lpn_index]], (void *)virt_addr, (void *)(pg_cc_addr + page_offset), page_len, is_write);
}

static void nvme_pcie_pg_cc_copy_sgl_single_page(struct spdk_nvme_qpair *qpair, struct nvme_request *req, uint64_t nlpn, uint64_t req_length, uint64_t req_offset, uint64_t lpn_index) {
	uint64_t pg_cc_addr;
	uint64_t len = req->payload_size;
	uint32_t page_len, page_offset;
	uint32_t page_size = 4096; 
	...
	uintptr_t virt_addr;
	uint32_t sgl_len;
	uint64_t forward_length = 0;

	if (lpn_index != 0) {
		forward_length = (page_size - (req_offset & page_mask)) + (lpn_index - 1) * page_size;
		req_offset += forward_length;
		len -= forward_length;
		assert((req_offset & page_mask) == 0);
	}

	reset_sgl_fn(sgl_cb_arg, req->payload_offset + forward_length);
	next_sge_fn(sgl_cb_arg, (void **)&virt_addr, &sgl_len);
	assert(sgl_len > 0);
	...
	uint64_t cp_len = spdk_min(page_len, sgl_len);
	nvme_pcie_copy_pg_cc(&qpair->ctrlr->host_mem_pg_lock[req->phy_pg_id[lpn_index]], (void *)virt_addr, (void *)(pg_cc_addr + page_offset), cp_len, is_write);

	virt_addr = virt_addr + cp_len;
	req_offset += cp_len;
	assert(len >= cp_len);
	len -= cp_len;
	page_len -= cp_len;
	...
	if (!sgl_len && len > 0) {
		...
		if (page_len > 0) {
			assert(sgl_len > page_len);
			nvme_pcie_copy_pg_cc(&qpair->ctrlr->host_mem_pg_lock[req->phy_pg_id[lpn_index]], (void *)virt_addr, (void *)(pg_cc_addr + page_offset + cp_len), page_len, is_write);

			virt_addr = virt_addr + page_len;
			req_offset += page_len;
			assert(len >= page_len);
			len -= page_len;
			page_len -= page_len;
...
void nvme_pcie_pg_cc_copy_single_page(struct spdk_nvme_qpair *qpair, struct nvme_request *req, uint64_t lpn_index) {
	struct spdk_nvme_cmd *cmd = &req->cmd;

	assert(cmd->opc == SPDK_NVME_OPC_PG_CC_READ || cmd->opc == SPDK_NVME_OPC_PG_CC_WRITE);

	uint64_t lba = cmd->cdw10, lba_count = cmd->cdw12 & 0xFFFF;
	uint64_t req_length = (lba_count + 1) * 512, req_offset = lba * 512;
	...
	if (req->payload.reset_sgl_fn) {
		nvme_pcie_pg_cc_copy_sgl_single_page(qpair, req, nlpn, req_length, req_offset, lpn_index);
	} else {
		nvme_pcie_pg_cc_copy_prp_single_page(qpair, req, nlpn, req_length, req_offset, lpn_index);
	}
}

static void nvme_pcie_pg_cc_copy_sgl(struct spdk_nvme_qpair *qpair, struct nvme_request *req, uint64_t nlpn, uint64_t req_length, uint64_t req_offset) {
	uint64_t pg_cc_addr;
	uint64_t len = req->payload_size;
	uint32_t page_len, page_offset;
	uint32_t page_size = 4096; 
	...
	reset_sgl_fn(sgl_cb_arg, req->payload_offset);
	next_sge_fn(sgl_cb_arg, (void **)&virt_addr, &sgl_len);
	assert(sgl_len > 0);
	...
	for (uint64_t i = 0; i < nlpn; i++) {
		...
		pg_cc_addr = trans_pgid2addr(qpair->ctrlr, req->phy_pg_id[i]);

		//printf("sgl_cb_arg=%lx, virt_addr=%lx, pg_cc_addr=%lx, page_len=%d, sgl_len=%u, req_offset=%lu\n", (uint64_t)sgl_cb_arg, virt_addr, pg_cc_addr, page_len, sgl_len, req_offset);

		uint64_t cp_len = spdk_min(page_len, sgl_len);

		if (!req->hit_in_driver[i])
			nvme_pcie_copy_pg_cc(&qpair->ctrlr->host_mem_pg_lock[req->phy_pg_id[i]], (void *)virt_addr, (void *)(pg_cc_addr + page_offset), cp_len, is_write);
		...
		if (!sgl_len && len > 0) {
			...
			if (page_len > 0) {
				...
				if (!req->hit_in_driver[i])
					nvme_pcie_copy_pg_cc(&qpair->ctrlr->host_mem_pg_lock[req->phy_pg_id[i]], (void *)virt_addr, (void *)(pg_cc_addr + page_offset + cp_len), page_len, is_write);
...
static void nvme_pcie_pg_cc_copy_prp(struct spdk_nvme_qpair *qpair, struct nvme_request *req, uint64_t nlpn, uint64_t req_length, uint64_t req_offset) {
	uint64_t pg_cc_addr;
	uint64_t len = req->payload_size;
	uint32_t page_len, page_offset;
	uint32_t page_size = 4096; 
	...
	for (uint64_t i = 0; i < nlpn; i++) {
		...
		if (!req->hit_in_driver[i])
			nvme_pcie_copy_pg_cc(&qpair->ctrlr->host_mem_pg_lock[req->phy_pg_id[i]], (void *)virt_addr, (void *)(pg_cc_addr + page_offset), page_len, is_write);
...
void nvme_pcie_pg_cc_copy_data(struct spdk_nvme_qpair *qpair, struct nvme_tracker *tr) {
	struct nvme_request *req = tr->req;
	...
	if (req->payload.reset_sgl_fn) {
		nvme_pcie_pg_cc_copy_sgl(qpair, req, nlpn, req_length, req_offset);
	} else {
		nvme_pcie_pg_cc_copy_prp(qpair, req, nlpn, req_length, req_offset);
	}
}

void nvme_pcie_pg_cc_parse_pg_id(struct spdk_nvme_qpair *qpair, struct nvme_tracker *tr, 
																				uint32_t page_size, struct spdk_nvme_cpl *cpl) {
	struct nvme_request *req = tr->req;
	uint64_t len = req->payload_size;
	uint32_t seg_len;
	int pg_num = 0;
	...
	SPDK_DEBUGLOG(nvme, "nvme_pcie_pg_cc_parse_pg_id, len %lu, page_size %u\n", 
							len, page_size);
	
	if (req->hit_in_driver[0]) {
		assert(cpl->res64 == INVALID_PG_ID);
		pg_num++;
	} else {
		assert(cpl->res64 != INVALID_PG_ID);
		req->phy_pg_id[pg_num++] = cpl->res64;
	}

	seg_len = spdk_min(page_size - (req_offset & page_mask), len);
	len -= seg_len;
	...
	while (len) {
		seg_len = spdk_min(page_size, len);
		pg_id = tr->u.prp[i];
		len -= seg_len;
		if (req->hit_in_driver[pg_num]) {
			assert(pg_id == INVALID_PG_ID);
			pg_num++;
		} else {
			assert(pg_id != INVALID_PG_ID);
			req->phy_pg_id[pg_num++] = pg_id;
		}
		i++;
	}
	...
	assert((uint64_t)pg_num == end_lpn - start_lpn + 1);
	assert(pg_num <= 504);
...
void
nvme_pcie_qpair_complete_tracker_hit_in_driver(struct spdk_nvme_qpair *qpair, struct nvme_tracker *tr, struct spdk_nvme_cpl *cpl, bool print_on_error)
{
	struct nvme_pcie_qpair		*pqpair = nvme_pcie_qpair(qpair);
	struct nvme_request		*req;
	...
	spdk_trace_record(TRACE_NVME_PCIE_COMPLETE, qpair->id, 0, (uintptr_t)req, req->cb_arg,
			  (uint32_t)req->cmd.cid, (uint32_t)0);
...
void
nvme_pcie_process_cache_completion(struct spdk_nvme_qpair *qpair, struct nvme_tracker *tr)
{
	struct nvme_request *req = tr->req, *pending_req;
	uint64_t lba = req->cmd.cdw10, lba_count = req->cmd.cdw12 & 0xFFFF;
	uint64_t req_length = (lba_count + 1) * 512, req_offset = lba * 512;
	
	uint64_t start_lpn = req_offset / PAGE_SIZE;
	uint64_t end_lpn = (req_offset + req_length - 1) / PAGE_SIZE;

	struct page_table_cacheline *cacheline;
	struct unpacked_pte_header pte;

	uint64_t not_hit_page = 0;
	uint64_t cacheline_pos, cacheline_offset;

	assert(req->pending_pg_num > 0);
	req->pending_pg_num--;

	for (uint64_t vir_pg_id = start_lpn, i = 0; vir_pg_id <= end_lpn; vir_pg_id++, i++) {
		if (req->hit_in_driver[i]) {
			continue ;
		}
		...
		cacheline = get_cacheline(qpair, vir_pg_id);
		pte = get_pte(qpair, vir_pg_id, cacheline);
		
		assert(pte.exist);
		assert(pte.busy);

		unset_busy_and_set_phy_pg_id(qpair, vir_pg_id, req->phy_pg_id[i], cacheline);

		cacheline_pos = get_cacheline_pos(cacheline);
		cacheline_offset = vir_pg_id % CACHELINE_PACK_PAGES;

		struct spdk_nvme_pending_req_entry *entry, *tmp;
		STAILQ_FOREACH_SAFE(entry, &qpair->pending_req[cacheline_pos][cacheline_offset], stailq, tmp) {

			pending_req = entry->req;
			assert(pending_req);

			uint64_t lba = pending_req->cmd.cdw10;
			uint64_t req_offset = lba * 512;
			uint64_t pending_start_lpn = req_offset / PAGE_SIZE;

			assert(vir_pg_id >= pending_start_lpn);
			assert(pending_req->phy_pg_id[vir_pg_id - pending_start_lpn] == INVALID_PG_ID);
			pending_req->phy_pg_id[vir_pg_id - pending_start_lpn] = req->phy_pg_id[i];

			nvme_pcie_pg_cc_copy_single_page(qpair, pending_req, vir_pg_id - pending_start_lpn);

			assert(pending_req->pending_pg_num > 0);
			pending_req->pending_pg_num--;

			if (pending_req->pending_pg_num == 0) {
				STAILQ_INSERT_HEAD(&qpair->hit_req, pending_req, hit_stailq);
			}
		}

		STAILQ_FOREACH_SAFE(entry, &qpair->pending_req[cacheline_pos][cacheline_offset], stailq, tmp) {
			entry->req = NULL;
			STAILQ_REMOVE(&qpair->pending_req[cacheline_pos][cacheline_offset], entry, spdk_nvme_pending_req_entry, stailq);
			STAILQ_INSERT_TAIL(&qpair->free_pending_req_head, entry, stailq);
		}

		// pin_page_with_hint(qpair, vir_pg_id, NULL, cacheline);
	}

	assert(not_hit_page == (uint64_t)req->total_need_pg_num);
}
...
nvme_pcie_qpair_process_completions(struct spdk_nvme_qpair *qpair, uint32_t max_completions)
{
	...
	// TODO: add a check for the queue of cached requests and process them

	if (!nvme_qpair_is_admin_queue(qpair)) {
		struct spdk_nvme_cpl fake_cpl;
		memset(&fake_cpl, 0, sizeof(struct spdk_nvme_cpl));


		while (!STAILQ_EMPTY(&qpair->hit_req)) {
			req = STAILQ_FIRST(&qpair->hit_req);
			STAILQ_REMOVE_HEAD(&qpair->hit_req, hit_stailq);
			tr = &pqpair->tr[req->cmd.cid];

			nvme_pcie_set_time(&(req->stat.req_pending_end_time));

			switch (req->cmd.opc) {
				case SPDK_NVME_OPC_PG_CC_WRITE:
				case SPDK_NVME_OPC_PG_CC_READ:
					// TODO: support write data to the page cache
					// nvme_pcie_process_cache_completion(qpair, tr, req);
					nvme_pcie_set_time(&tr->req->stat.postcache_start_time);
					nvme_pcie_pg_cc_copy_data(qpair, tr);
					// nvme_pcie_qpair_unpin_rw_callback(ctrlr, qpair, tr);
					nvme_pcie_set_time(&tr->req->stat.postcache_end_time);
					break;
				case SPDK_NVME_OPC_UN_PIN:
					// nvme_pcie_process_unpin_completion(qpair, tr, req);
					assert(0);
					break;
				default:
			...
			spdk_add_statis(qpair, req);

			fake_cpl.cid = req->cmd.cid;
			fake_cpl.sqid = qpair->id;
			/* REMIND: hack to set status zero */
			fake_cpl.status_raw = 0;


			nvme_pcie_qpair_complete_tracker_hit_in_driver(qpair, tr, &fake_cpl, true);
			num_completions++;
		}
	}
	...
	while (1) {
		...
		if (tr->req) {
			if (!nvme_qpair_is_admin_queue(qpair)) {
				if (is_pg_cc_io_request(tr->req->cmd.opc)) {
					nvme_pcie_set_time(&tr->req->stat.postcache_start_time);
					nvme_pcie_pg_cc_parse_pg_id(qpair, tr, 4096, cpl);
					nvme_pcie_process_cache_completion(qpair, tr);
				}
			}
			
			if (is_pg_cc_io_request(tr->req->cmd.opc)) {
				if (tr->req->pending_pg_num == 0) {
					run_complete = true;
					assert(!nvme_qpair_is_admin_queue(qpair));
					nvme_pcie_pg_cc_copy_data(qpair, tr);
					// nvme_pcie_qpair_unpin_rw_callback(ctrlr, qpair, tr);
					nvme_pcie_set_time(&tr->req->stat.postcache_end_time);
					spdk_add_statis(qpair, tr->req);
					nvme_pcie_qpair_complete_tracker(qpair, tr, cpl, true);
				} else {
					assert(tr->req->stat.req_pending_start_time == 0);
					assert(!nvme_qpair_is_admin_queue(qpair));
					nvme_pcie_set_time(&(tr->req->stat.req_pending_start_time));
				}
			} else {
				run_complete = true;
				if (!nvme_qpair_is_admin_queue(qpair)) {
			...
			SPDK_ERRLOG("SQ id %hu, cmd_id %hu, cq_head %hu, cpl does not map to outstanding cmd\n", 
										qpair->id, cpl->cid, pqpair->cq_head);
			spdk_nvme_qpair_print_completion(qpair, cpl);
			raise(SIGTRAP);
			assert(0);
		}
			  ...
			  uint32_t page_size)
{
	...
	SPDK_DEBUGLOG(nvme, "nvme_pcie_prp_list_append virt_addr: %p, len: %u, page_size: %u\n", 
											virt_addr, (uint32_t)len, page_size);

	if (cmd->opc != SPDK_NVME_OPC_PG_CC_READ 
		&& cmd->opc != SPDK_NVME_OPC_PG_CC_WRITE
		&& cmd->opc != SPDK_NVME_OPC_UN_PIN) {
		i = *prp_index;
		while (len) {
			uint32_t seg_len;
		...
		*prp_index = i;
		...
		cmd->dptr.prp.prp1 = tr->prp_sgl_bus_addr;
		cmd->dptr.prp.prp2 = 0;
		SPDK_DEBUGLOG(nvme, "PG_CC_OP set prp1 = %p\n", (void *)cmd->dptr.prp.prp1);
	}
		...
		struct nvme_tracker *tr, bool dword_aligned)
{
	...
	printf("nvme_pcie_qpair_build_contig_hw_sgl_request req->payload_offset: %u, req->payload_size: %u\n", 
