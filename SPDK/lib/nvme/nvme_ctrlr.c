			   ...
			   const struct spdk_nvme_io_qpair_opts *opts)
{
	...
	if (ctrlr->nr_host_mem_descs > 0)
		initialize_page_table_cache(qpair);
...
int 
spdk_nvme_ctrlr_free_host_memory_buffer(struct spdk_nvme_ctrlr *ctrlr) {
	int nr_host_mem_descs = ctrlr->nr_host_mem_descs;
	struct spdk_nvme_host_mem_buf_desc *descs = ctrlr->host_mem_descs;
	void **bufs = ctrlr->host_mem_desc_bufs;

	if (!bufs) {
		return 0;
	}

	for (int i = 0; i < nr_host_mem_descs; i++) {
		if (!bufs[i]) {
			continue;
		}
...
spdk_nvme_ctrlr_free_io_qpair(struct spdk_nvme_qpair *qpair)
{
	...
	// printf("begin destory_page_table_cache\n");
	// destroy_page_table_cache(qpair);
	// printf("end destory_page_table_cache\n");
...
nvme_ctrlr_identify_done(void *arg, const struct spdk_nvme_cpl *cpl)
{
	...
	ctrlr->ftl_num = ((uint32_t *)ctrlr->cdata.vs)[0];
	assert(ctrlr->ftl_num > 0);
	assert(ctrlr->ftl_num <= SPDK_NVME_MAX_FTL_NUM);
	ctrlr->shard_pg_num = ((uint32_t *)ctrlr->cdata.vs)[1];
	assert(ctrlr->shard_pg_num > 0);
...
void
nvme_ctrlr_free_host_mem_buf(struct spdk_nvme_ctrlr *ctrlr) {
	printf("begin nvme_ctrlr_free_host_mem_buf\n");
	
	for (uint32_t i = 0; i < ctrlr->nr_host_mem_descs; i++) {
		spdk_free(ctrlr->host_mem_desc_bufs[i]);
	}

	spdk_free(ctrlr->host_mem_desc_bufs);

	printf("end nvme_ctrlr_free_host_mem_buf\n");
}
...
uint64_t
spdk_nvme_ctrlr_get_cachesize_per_qpair(const struct spdk_nvme_ctrlr *ctrlr)
{
	assert(ctrlr->max_io_qpairs > 0);
	return (ctrlr->host_mem_size / 4096 - 4096) / ctrlr->max_io_qpairs;
}

uint64_t
spdk_nvme_ctrlr_get_cachesize_per_qpair_per_ftl(const struct spdk_nvme_ctrlr *ctrlr)
{
	assert(ctrlr->max_io_qpairs > 0);
	assert(ctrlr->ftl_num > 0);
	return spdk_nvme_ctrlr_get_cachesize_per_qpair(ctrlr) / ctrlr->ftl_num;
}

uint32_t
spdk_nvme_ctrlr_get_ftl_num(const struct spdk_nvme_ctrlr *ctrlr)
{
	assert(ctrlr->ftl_num > 0);
	return ctrlr->ftl_num;
}

uint32_t
spdk_nvme_ctrlr_get_shard_pg_num(const struct spdk_nvme_ctrlr *ctrlr)
{
	assert(ctrlr->shard_pg_num > 0);
	return ctrlr->shard_pg_num;
}

uint64_t
spdk_nvme_ctrlr_trans_pgid2ftlid(const struct spdk_nvme_ctrlr *ctrlr, uint32_t pg)
{
	assert(ctrlr);
	assert(ctrlr->ftl_num > 0);
	assert(ctrlr->shard_pg_num > 0);
	return (pg / ctrlr->shard_pg_num) % ctrlr->ftl_num;
}
