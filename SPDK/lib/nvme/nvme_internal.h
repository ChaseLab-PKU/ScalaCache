...
#include "lib/nvme/nvme_host_mem_cache.h"
#include "lib/nvme/nvme_statistator.h"
...
#define SPDK_NVME_MAX_FTL_NUM (16)
...
struct nvme_request {
	...
	bool hit_in_driver[NVME_MAX_PRP_LIST_ENTRIES + 6];
	uint16_t origin_opc;
	uint64_t phy_pg_id[NVME_MAX_PRP_LIST_ENTRIES + 6];
	uint8_t pin_count[NVME_MAX_PRP_LIST_ENTRIES + 6];
	uint32_t total_need_pg_num;
	uint32_t pending_pg_num;
	uint16_t need_pg_num[SPDK_NVME_MAX_FTL_NUM];
	bool is_pre_unpin;
	STAILQ_ENTRY(nvme_request)	hit_stailq;

	struct spdk_stat {
		uint64_t req_sending_start_time;
		uint64_t req_sending_end_time;
		uint64_t req_pending_start_time;
		uint64_t req_pending_end_time;
		uint64_t precache_start_time;
		uint64_t precache_end_time;
		uint64_t postcache_start_time;
		uint64_t postcache_end_time;
		uint64_t pg_num;
		uint64_t pg_hit_num;
		uint64_t pg_miss_num;
	} stat;
...
struct spdk_nvme_pending_req_entry {
	struct nvme_request *req;
	STAILQ_ENTRY(spdk_nvme_pending_req_entry) stailq;
};
...
struct spdk_nvme_qpair {
	...
	STAILQ_HEAD(, nvme_request)		hit_req;
	...
	struct page_table_cacheline *cacheline_store;
	/* TODO: change the pending_req to linking list */
	STAILQ_HEAD(, spdk_nvme_pending_req_entry) **pending_req;
	struct page_status *page_status;
	uint64_t cacheline_size;
	uint64_t cacheline_bucket_size;
	uint64_t pin_size[16];
	uint64_t mapping_size[16];
	uint64_t victim_bucket;

	STAILQ_HEAD(, spdk_nvme_pending_req_entry) free_pending_req_head;
	struct spdk_nvme_pending_req_entry *free_pending_req_entry;
	uint64_t free_pending_req_size;

	struct spdk_statistator *stat;

	STAILQ_HEAD(, page_table_cacheline) cacheline_pool;
};

struct spdk_nvme_host_mem_buf_desc {
	uint64_t vir_addr;
	uint64_t phy_addr;
	uint32_t size;
	uint32_t rsvd;
...
struct spdk_nvme_ctrlr {
	...
	uint32_t 											num_io_qpairs;
	uint32_t											max_io_qpairs;
	...
	/* Host Memory Buffer suppoert */
	uint32_t	nr_host_mem_descs;
	uint64_t	host_mem_size;
	uint64_t		host_mem_descs_dma;
	struct spdk_nvme_host_mem_buf_desc *host_mem_descs;
	void **		host_mem_desc_bufs;
	bool host_mem_is_existed;
	uint64_t  *host_mem_vir_addr;
	uint64_t   host_mem_pg_size;
	pthread_rwlock_t *host_mem_pg_lock;

	uint32_t ftl_num;
	uint32_t shard_pg_num;
...
void nvme_ctrlr_free_host_mem_buf(struct spdk_nvme_ctrlr *ctrlr);
...
static inline void nvme_reset_req_time_stat(struct nvme_request *req)
{
	req->stat.req_sending_start_time = spdk_get_ticks();
	req->stat.req_sending_end_time = 0;
	req->stat.req_pending_start_time = req->stat.req_pending_end_time = 0;
	req->stat.precache_start_time = req->stat.precache_end_time = 0;
	req->stat.postcache_start_time = req->stat.postcache_end_time = 0;
	req->stat.pg_num = req->stat.pg_hit_num = req->stat.pg_miss_num = 0;
}
		      ...
		      spdk_nvme_cmd_cb cb_fn, void *cb_arg)
{
	...
	req->is_pre_unpin = false;

	nvme_reset_req_time_stat(req);
