...
static inline struct nvme_request *
_nvme_ns_cmd_pg_rw(struct spdk_nvme_ns *ns, struct spdk_nvme_qpair *qpair,
			const struct nvme_payload *payload, uint32_t payload_offset, uint32_t md_offset,
			uint64_t lba, uint32_t lba_count, spdk_nvme_cmd_cb cb_fn, void *cb_arg, uint32_t opc,
			uint32_t io_flags, uint16_t apptag_mask, uint16_t apptag, bool check_sgl, int *rc) 
{
...
static inline struct nvme_request *
_nvme_ns_cmd_pg_cc_unpin(struct spdk_nvme_ns *ns, struct spdk_nvme_qpair *qpair,
						uint64_t lba, uint32_t lba_count, spdk_nvme_cmd_cb cb_fn, void *cb_arg, int *rc) {
	struct nvme_request *req;

	req = nvme_allocate_request(qpair, NULL, 0, 0, cb_fn, cb_arg);
	if (req == NULL) {
		*rc = -ENOMEM;
		return NULL;
	}
	...
	//TODO: not support unpin cmd from host util supporting mmap
	assert(0);

	_nvme_ns_cmd_setup_request(ns, req, SPDK_NVME_OPC_UN_PIN, lba, lba_count, 0, 0, 0);
	return req;
}
		      ...
		      uint32_t io_flags)
{
	...
	uint32_t opc = SPDK_NVME_OPC_READ;
	if (ctrlr->nr_host_mem_descs > 0) {
		opc = SPDK_NVME_OPC_PG_CC_READ;
	}

	req = _nvme_ns_cmd_rw(ns, qpair, &payload, 0, 0, lba, lba_count, cb_fn, cb_arg, opc,
			      io_flags, 0,
			      0, false, &rc);
	if (req != NULL) {
		return nvme_qpair_submit_request(qpair, req);
...
int 
spdk_nvme_ns_cmd_pg_cc_read_with_md(struct spdk_nvme_ns *ns, struct spdk_nvme_qpair *qpair, void *buffer,
						void *metadata,
						uint64_t lba, 
						uint32_t lba_count, spdk_nvme_cmd_cb cb_fn, void *cb_arg,
						uint32_t io_flags, uint16_t apptag_mask, uint16_t apptag)
{
	...
	req = _nvme_ns_cmd_rw(ns, qpair, &payload, 0, 0, lba, lba_count, cb_fn, cb_arg, SPDK_NVME_OPC_PG_CC_READ,
...
int 
spdk_nvme_ns_cmd_pg_cc_readv_with_md(struct spdk_nvme_ns *ns, struct spdk_nvme_qpair *qpair,
			       uint64_t lba, uint32_t lba_count,
			       spdk_nvme_cmd_cb cb_fn, void *cb_arg, uint32_t io_flags,
			       spdk_nvme_req_reset_sgl_cb reset_sgl_fn,
			       spdk_nvme_req_next_sge_cb next_sge_fn, void *metadata,
		       ...
		       uint32_t io_flags)
{
	...
	uint32_t opc = SPDK_NVME_OPC_WRITE;
	if (ctrlr->nr_host_mem_descs > 0) {
		opc = SPDK_NVME_OPC_PG_CC_WRITE;
	}
...
int spdk_nvme_ns_cmd_pg_cc_write(struct spdk_nvme_ns *ns, struct spdk_nvme_qpair *qpair,
													void *buffer, uint64_t lba,
													uint32_t lba_count, spdk_nvme_cmd_cb cb_fn, void *cb_arg,
													uint32_t io_flags)
{
	...
	req = _nvme_ns_cmd_pg_rw(ns, qpair, &payload, 0, 0, lba, lba_count, cb_fn, cb_arg, SPDK_NVME_OPC_PG_CC_WRITE,
													 io_flags, 0, 0, false, &rc);

	if (req != NULL)
		return nvme_qpair_submit_request(qpair, req);

	printf("nvme_ns_cmd_pg_cc_write: req is NULL\n");
	assert(0);
	return 0;
}

int spdk_nvme_ns_cmd_pg_cc_read(struct spdk_nvme_ns *ns, struct spdk_nvme_qpair *qpair,
													void *buffer, uint64_t lba,
													uint32_t lba_count, spdk_nvme_cmd_cb cb_fn, void *cb_arg,
													uint32_t io_flags)
{
	...
	req = _nvme_ns_cmd_pg_rw(ns, qpair, &payload, 0, 0, lba, lba_count, cb_fn, cb_arg, SPDK_NVME_OPC_PG_CC_READ,
													 io_flags, 0, 0, false, &rc);
...
int spdk_nvme_ns_cmd_pg_cc_unpin(struct spdk_nvme_ns *ns, struct spdk_nvme_qpair *qpair, 
														uint64_t lba, uint32_t lba_count, uint16_t origin_opc, spdk_nvme_cmd_cb cb_fn, void *cb_arg)
{
	struct nvme_request *req;
	struct nvme_payload payload;
	int rc = 0;
	...
	req = _nvme_ns_cmd_pg_rw(ns, qpair, &payload, 0, 0, lba, lba_count, cb_fn, cb_arg, SPDK_NVME_OPC_UN_PIN,
													 0, 0, 0, false, &rc);

	if (req != NULL) {
		assert(origin_opc == SPDK_NVME_OPC_PG_CC_WRITE || origin_opc == SPDK_NVME_OPC_PG_CC_READ);
		if (req->num_children == 0)
			req->origin_opc = origin_opc;
		else {
			struct nvme_request	*child_req, *tmp;
			TAILQ_FOREACH_SAFE(child_req, &req->children, child_tailq, tmp) {
				child_req->origin_opc = origin_opc;
			}
		}
		return nvme_qpair_submit_request(qpair, req);
	}
	...
	printf("nvme_ns_cmd_pg_cc_unpin: req is NULL\n");
	assert(0);
	return 0;
}
...
int
spdk_nvme_ns_cmd_pg_cc_write_with_md(struct spdk_nvme_ns *ns, struct spdk_nvme_qpair *qpair,
			       ...
			       uint32_t io_flags, uint16_t apptag_mask, uint16_t apptag)
{
	...
	req = _nvme_ns_cmd_rw(ns, qpair, &payload, 0, 0, lba, lba_count, cb_fn, cb_arg, SPDK_NVME_OPC_PG_CC_WRITE,
			      io_flags, apptag_mask, apptag, false, &rc);
	if (req != NULL) {
		return nvme_qpair_submit_request(qpair, req);
	} else {
...
int
spdk_nvme_ns_cmd_pg_cc_writev_with_md(struct spdk_nvme_ns *ns, struct spdk_nvme_qpair *qpair,
				uint64_t lba, uint32_t lba_count,
				spdk_nvme_cmd_cb cb_fn, void *cb_arg, uint32_t io_flags,
				spdk_nvme_req_reset_sgl_cb reset_sgl_fn,
				spdk_nvme_req_next_sge_cb next_sge_fn, void *metadata,
