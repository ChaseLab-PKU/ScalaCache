		       ...
		       void *cb_arg)
{
	...
	struct spdk_fio_thread		*fio_thread = td->io_ops_data;
	struct spdk_fio_target *target = fio_req->io->file->engine_data;
...
spdk_fio_queue(struct thread_data *td, struct io_u *io_u)
{
	...
	struct spdk_bdev *bdev = spdk_bdev_desc_get_bdev(target->desc);
	uint64_t ssd_size = spdk_bdev_get_num_blocks(bdev) * spdk_bdev_get_block_size(bdev);

	switch (io_u->ddir) {
	case DDIR_READ:
	case DDIR_WRITE:
		if (io_u->offset + io_u->xfer_buflen > ssd_size) {
			io_u->offset %= ssd_size;
			assert(io_u->offset + io_u->xfer_buflen < ssd_size);
		}
		break;
	default:
		break;
