...
write_complete(void *arg, const struct spdk_nvme_cpl *completion)
{
	...
	rc = spdk_nvme_ns_cmd_pg_cc_read(ns_entry->ns, ns_entry->qpair, sequence->buf,
				   0, /* LBA start */
				   1, /* number of LBAs */
...
hello_world(void)
{
	...
	TAILQ_FOREACH(ns_entry, &g_namespaces, link) {
		 ...
		 *  process.
		 */
		rc = spdk_nvme_ns_cmd_pg_cc_write(ns_entry->ns, ns_entry->qpair, sequence.buf,
					    0, /* LBA start */
					    1, /* number of LBAs */
...
static void
set_host_mem_buffer_completion(void *cb_arg, const struct spdk_nvme_cpl *cpl)
{
	if (spdk_nvme_cpl_is_error(cpl)) {
		printf("nvme set_host_memory_buffer completion error\n");
	}
	outstanding_commands--;
}

static void 
nvme_ctrlr_cmd_set_host_memory_buffer(struct spdk_nvme_ctrlr *ctrlr) 
{
	int rc;
	outstanding_commands = 0;
	rc = spdk_nvme_ctrlr_cmd_set_host_memory_buffer(ctrlr, set_host_mem_buffer_completion, NULL);
	if (rc == 0) {
		outstanding_commands++;
	} else {
		printf("nvme set_host_memory_buffer error %d\n", rc);
	}
	  ...
	  struct spdk_nvme_ctrlr *ctrlr, const struct spdk_nvme_ctrlr_opts *opts)
{
	...
	if (cdata->hmpre) {
		printf("device support HMB, cdata->hmpre %u\n", cdata->hmpre);
		printf("start to initialize HMB\n");
		nvme_ctrlr_cmd_set_host_memory_buffer(ctrlr);
	} else {
		printf("device does not support HMB\n");
		assert(0);
	}
