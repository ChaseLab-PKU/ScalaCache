...
struct ns_worker_stats {
	...
	uint64_t		rw_max_tsc[2];
	uint64_t		rw_min_tsc[2];
	uint64_t		rw_total_tsc[2];
...
static int admin_outstanding_commands;
...
static int
nvme_submit_io(struct perf_task_context *task_context, struct ns_worker_ctx *ns_ctx,
	       struct ns_entry *entry)
{
	...
	if (mode != DIF_MODE_NONE) {
		rc = spdk_dif_ctx_init(&task->dif_ctx, entry->block_size, entry->md_size,
				       ...
				       start_lba, 0xFFFF, (uint16_t)entry->io_size_blocks, 0, 0);
		if (rc != 0) {
	...
	task_context->ns = entry->u.nvme.ns;
	task_context->qpair = ns_ctx->u.nvme.qpair[qp_num];
	task_context->lba = start_lba;
	task_context->io_size_blocks = lba_count;

	//printf("submit io %s, lba %lu io_size_blocks %lu\n", task->is_read ? "read" : "write", start_lba, lba_count);

	if (task->is_read) {
		if (task->iovcnt == 1) {
			if (g_host_mem_buf) {
				return spdk_nvme_ns_cmd_pg_cc_read_with_md(entry->u.nvme.ns, ns_ctx->u.nvme.qpair[qp_num],
							     task->iovs[0].iov_base, task->md_iov.iov_base,
							     start_lba,
							     lba_count, io_complete,
							     task_context, entry->io_flags,
			...
			if (g_host_mem_buf) {
				return spdk_nvme_ns_cmd_pg_cc_readv_with_md(entry->u.nvme.ns, ns_ctx->u.nvme.qpair[qp_num],
							      start_lba, lba_count,
							      io_complete, task_context, entry->io_flags,
		...
		switch (mode) {
		case DIF_MODE_DIF:
			rc = spdk_dif_generate(task->iovs, task->iovcnt, lba_count, &task->dif_ctx);
			if (rc != 0) {
			...
			break;
		case DIF_MODE_DIX:
			rc = spdk_dix_generate(task->iovs, task->iovcnt, &task->md_iov, lba_count,
					       &task->dif_ctx);
			if (rc != 0) {
		...
		if (task->iovcnt == 1) {
			if (g_host_mem_buf) {
				return spdk_nvme_ns_cmd_pg_cc_write_with_md(entry->u.nvme.ns, ns_ctx->u.nvme.qpair[qp_num],
							      task->iovs[0].iov_base, task->md_iov.iov_base,
							      start_lba,
							      lba_count, io_complete,
							      task_context, entry->io_flags,
			...
			if (g_host_mem_buf) {
				return  spdk_nvme_ns_cmd_pg_cc_writev_with_md(entry->u.nvme.ns, ns_ctx->u.nvme.qpair[qp_num],
							       start_lba, lba_count,
							       io_complete, task_context, entry->io_flags,
							       nvme_perf_reset_sgl, nvme_perf_next_sge,
							       task->md_iov.iov_base,
...
submit_single_io(struct perf_task_context *task_context)
{
	...
	// printf("Worker %u, submit io: %lu %lu %d\n", ns_ctx->worker_id, task->start_lba, task->lba_count, task->is_read);
	...
	if (ns_ctx->stats.io_submitted % 100000 == 0) {
		// printf("Worker %u, submitted %lu\n", ns_ctx->worker_id, ns_ctx->stats.io_submitted);
	} else if (ns_ctx->stats.io_submitted >= 2188000) {
		// printf("Worker %u, submitted %lu, start_lba %lu, lba_count %lu\n", ns_ctx->worker_id, ns_ctx->stats.io_submitted, task->start_lba, task->lba_count);
	}
...
{
	struct perf_task_context *task_context = ctx;
	struct perf_task *task = task_context->task;
	...
	task_complete(task_context);
}

static void
hmb_init_complete(void *ctx, const struct spdk_nvme_cpl *cpl)
{
	if (spdk_unlikely(spdk_nvme_cpl_is_error(cpl))) {
		RATELIMIT_LOG("HMB init completed with error (sct=%d, sc=%d)\n",
			      cpl->status.sct, cpl->status.sc);
	}
	admin_outstanding_commands--;
}
...
submit_io(struct ns_worker_ctx *ns_ctx, int queue_depth)
{
	...
	printf("Worker %d: size_in_ios: %lu\n", ns_ctx->worker_id, ns_ctx->entry->size_in_ios);
...
static void
print_periodic_performance(uint64_t last_print_tsc, uint64_t current_print_tsc, bool warmup)
{
	uint64_t io_this_second;
	double mb_this_second = 0;
	...
	TAILQ_FOREACH(worker, &g_workers, link) {
		...
		TAILQ_FOREACH(ns_ctx, &worker->ns_ctx, link) {
			...
			mb_this_second += ns_ctx->stats.io_lba_completed - ns_ctx->stats.last_io_lba_completed;
			ns_ctx->stats.last_io_lba_completed = ns_ctx->stats.io_lba_completed;
	...
	uint64_t elapsed_time_in_usec =  ((current_print_tsc - last_print_tsc) * SPDK_SEC_TO_USEC / g_tsc_rate);
	printf("elapsed_time: %lu\n", elapsed_time_in_usec);
	mb_this_second = (double)mb_this_second * 512 / (1024 * 1024) * 1000 * 1000 / elapsed_time_in_usec;
...
work_fn(void *arg)
{
	...
	if (worker->lcore == g_main_core) {
		g_tsc_start = spdk_get_ticks();
	}
	tsc_current = g_tsc_start;
	tsc_next_print = tsc_current + g_print_period_in_ticks;
		...
		if (ns_ctx->is_draining) {
			if (warmup) {
				...
				TAILQ_FOREACH(ns_ctx, &worker->ns_ctx, link) {
					...
					ns_ctx->stats.rw_min_tsc[0] = UINT64_MAX;
					ns_ctx->stats.rw_min_tsc[1] = UINT64_MAX;
			...
			if (ns_ctx->stats.io_completed != 0) {
				...
				for (int op = 0; op < 2; op++) {
					if (ns_ctx->stats.rw_io_completed[op] != 0) {
						...
						rw_min_latency[op] = (double)ns_ctx->stats.rw_min_tsc[op] * 1000 * 1000 / g_tsc_rate;
						rw_max_latency[op] = (double)ns_ctx->stats.rw_max_tsc[op] * 1000 * 1000 / g_tsc_rate;
	...
	if (ns_count != 0 && total_io_completed) {
		...
		for (int op = 0; op < 2; op++) {
			if (total_rw_io_completed[op] != 0) {
				rw_sum_ave_latency[op] = ((double)total_rw_io_tsc[op] / total_rw_io_completed[op]) * 1000 * 1000 / g_tsc_rate;
				printf("%-*s: %10.2f %10.2f %10.2f %10.2f %10.2f\n",
	  ...
	  struct spdk_nvme_ctrlr *ctrlr, const struct spdk_nvme_ctrlr_opts *opts)
{
	...
	if (trid->trtype != SPDK_NVME_TRANSPORT_PCIE) {
		...
		if (cdata->hmpre && g_host_mem_buf) {
			int rc = spdk_nvme_ctrlr_cmd_set_host_memory_buffer(ctrlr, 
														hmb_init_complete, NULL);

			if (rc != 0) {
				printf("spdk_nvme_ctrlr_cmd_set_host_memory_buffer failed\n");
			} else {
				admin_outstanding_commands++;
			}

			while (admin_outstanding_commands) {
				spdk_nvme_ctrlr_process_admin_completions(ctrlr);
			}

			printf("HMB initialized\n");
		} else {
			printf("HMB not supported, please check\n");
		}
...
main(int argc, char **argv)
{
	...
	g_print_period_in_ticks = g_tsc_rate * 10;
	...
	g_elapsed_time_in_usec = (spdk_get_ticks() - g_tsc_start) * SPDK_SEC_TO_USEC / g_tsc_rate;
