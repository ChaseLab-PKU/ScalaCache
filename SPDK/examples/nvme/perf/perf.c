	       ...
	       struct ns_entry *entry, uint64_t offset_in_ios)
{
	...
	task_context->lba = lba;
	task_context->io_size_blocks = entry->io_size_blocks;
...
submit_single_io(struct perf_task_context *task_context)
{
	...
	rc = entry->fn_table->submit_io(task_context, ns_ctx, entry, offset_in_ios);
...
print_performance(void)
{
	...
	uint64_t total_rw_io_completed[2], total_rw_io_tsc[2];
			...
			if (ns_ctx->stats.io_completed != 0) {
				...
				for (int op = 0; op < 2; op++) {
					if (ns_ctx->stats.rw_io_completed[op] != 0) {
						...
						rw_mb_per_second[op] = rw_io_per_second[op] * g_io_size_bytes / (1024 * 1024);
